...
def authenticate_user():
    ...
    # Vérification si une image est fournie pour la reconnaissance faciale
    if image_file:
        ...
        try:
            ...
            if not comparison_result.get("verified", False):
                os.remove(temp_image_path)  # Nettoyer le fichier temporaire
                return jsonify({"error": "L'image fournie ne correspond pas à l'image modèle."}), 401

            # Si la comparaison est réussie, on supprime le fichier temporaire
